...
export function tickBrain(e, px, pz, dt, now, cfg = {}) {
  ...
  if (e.phase === PHASE.idle) {
    if (e.hold) {
      const ang = now * 0.7 + (e.sx || 0);
      const spd = (e.speed || 1.4) * 0.4;
      mesh.position.x += Math.cos(ang) * spd * dt;
      mesh.position.z += Math.sin(ang) * spd * dt;
    } else if (d < aggro && e.phaseT <= 0) {
    ...
    const glow = e.core || mesh;
    if (glow.material && glow.material.emissiveIntensity != null) {
      glow.material.emissiveIntensity = 1.1 + Math.sin(now * 18) * 0.5;
    }
    if (e.phaseT <= 0) {
